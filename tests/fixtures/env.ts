...
/** 去掉 .env 里常见的首尾单/双引号，避免把引号吃进 URL 或 Cookie 值 */
...
export function loadEnv(): E2EEnv {
  const rawCookie = process.env.CONSOLE_COOKIE || process.env.COOKIE || process.env.Cookie;
  return {
    ...
    consoleCookie: rawCookie ? stripEnvQuotes(rawCookie) : undefined,
    consoleLoginSuccessText: process.env.CONSOLE_LOGIN_SUCCESS_TEXT
      ? stripEnvQuotes(process.env.CONSOLE_LOGIN_SUCCESS_TEXT)
      : "欢迎登入系统~",
    consolePlatformVersion: process.env.CONSOLE_PLATFORM_VERSION
      ? stripEnvQuotes(process.env.CONSOLE_PLATFORM_VERSION)
      : "3.0.2603-beta.0315",
    consolePortalMenuText: process.env.CONSOLE_PORTAL_MENU_TEXT
      ? stripEnvQuotes(process.env.CONSOLE_PORTAL_MENU_TEXT)
      : "门户管理",
    consolePortalEntryName: process.env.CONSOLE_PORTAL_ENTRY_NAME
      ? stripEnvQuotes(process.env.CONSOLE_PORTAL_ENTRY_NAME)
      : "TERP 运营端",
    consolePortalConfigText: process.env.CONSOLE_PORTAL_CONFIG_TEXT
      ? stripEnvQuotes(process.env.CONSOLE_PORTAL_CONFIG_TEXT)
      : "门户配置",
    consolePublicPageMenuText: process.env.CONSOLE_PUBLIC_PAGE_MENU_TEXT
      ? stripEnvQuotes(process.env.CONSOLE_PUBLIC_PAGE_MENU_TEXT)
      : "公开页面",
    consoleScenePageName: process.env.CONSOLE_SCENE_PAGE_NAME
      ? stripEnvQuotes(process.env.CONSOLE_SCENE_PAGE_NAME)
      : "AT_公开页面",
    ...
    portalPublicSuccessText: process.env.PORTAL_PUBLIC_SUCCESS_TEXT
      ? stripEnvQuotes(process.env.PORTAL_PUBLIC_SUCCESS_TEXT)
      : undefined,
    portalNoPermissionText: process.env.PORTAL_NO_PERMISSION_TEXT
      ? stripEnvQuotes(process.env.PORTAL_NO_PERMISSION_TEXT)
      : undefined
  };
}
