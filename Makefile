...
help:
	@echo "make check   - 运行 workflow/check_quality.py（最新轮次 context/review.passed 等门禁）"
	@echo "make run     - 运行 workflow/run.py（task-state 守卫）"
	@echo "make doctor  - 依次执行 check + run（克隆后自检）"
	@echo "make init    - 生成本轮 harness 五件套骨架，需传 SUMMARY=kebab-case，例: make init SUMMARY=my-feature"
