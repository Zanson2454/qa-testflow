...
3. **本轮证据**：`harness/` 下按日期前缀对齐的四类文件 + 可选 `harness/contexts/`。

## 4. 开启新一轮迭代（最小步骤）

1. **Review**：阅读上一轮 `harness/handoffs/`、`harness/changes/`（若有）。
...
3. **Context**（建议）：在 `harness/contexts/` 新增本轮上下文说明。
...
文件命名约定：`YYYY-MM-DD-01-<type>-<summary>.md`，四类目录使用同一日期前缀表示同一轮。
